...
constexpr absl::string_view text{"application/connect-web-text"};
constexpr absl::string_view binary{"application/connect-web"};
...
class ConnectWebFilterIntegrationTest : public testing::TestWithParam<TestParams>,
                                     public HttpIntegrationTest {
public:
  ConnectWebFilterIntegrationTest()
      : HttpIntegrationTest(std::get<1>(GetParam()), std::get<0>(GetParam())) {}

  void SetUp() override {
    setUpstreamProtocol(Http::CodecType::HTTP2);
    config_helper_.prependFilter("name: envoy.filters.http.connect_web");
  }
  ...
  void testUpstreamDisconnect(const std::string& expected_connect_message_value,
    ...
    EXPECT_EQ(expected_connect_message_value, response->headers().getConnectMessageValue());
  ...
  void testBadUpstreamResponse(const std::string& start, const std::string& end,
    ...
    // Sending back non Connect-Web response.
    ...
    EXPECT_TRUE(response->complete());
    EXPECT_EQ(expected, response->headers().getConnectMessageValue());
...
INSTANTIATE_TEST_SUITE_P(
    Params, ConnectWebFilterIntegrationTest,
    ...
    ConnectWebFilterIntegrationTest::testParamsToString);

TEST_P(ConnectWebFilterIntegrationTest, ConnectWebTrailersNotDuplicated) {
  initialize();
  ...
  codec_client_->sendTrailers(*request_encoder_, request_trailers);
  waitForNextUpstreamRequest();
  default_response_headers_.setReferenceContentType(Http::Headers::get().ContentTypeValues.Connect);
...
TEST_P(ConnectWebFilterIntegrationTest, UpstreamDisconnect) {
  testUpstreamDisconnect("upstream connect error or disconnect/reset before headers. reset reason: "
                         "connection termination");
}

TEST_P(ConnectWebFilterIntegrationTest, UpstreamDisconnectWithLargeBody) {
...
TEST_P(ConnectWebFilterIntegrationTest, BadUpstreamResponse) {
  testBadUpstreamResponse("{", "\"percentage\": \"100%\"}",
                          /*expected=*/"{\"percentage\": \"100%25\"}");
}

TEST_P(ConnectWebFilterIntegrationTest, BadUpstreamResponseWithoutContentType) {
  testBadUpstreamResponse("{", "\"percentage\": \"100%\"}",
                          /*expected=*/"{\"percentage\": \"100%25\"}",
                          /*remove_content_type=*/true);
}

TEST_P(ConnectWebFilterIntegrationTest, BadUpstreamResponseLargeEnd) {
...
TEST_P(ConnectWebFilterIntegrationTest, BadUpstreamResponseLargeFirst) {

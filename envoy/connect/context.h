...
/**
 * Captures connect-related structures with cardinality of one per server.
 */
class Context {
public:
  virtual ~Context() = default;

  enum class Protocol { Connect, ConnectWeb };

  struct RequestStatNames;

  /**
   * Parses out request connect service-name and method from the path, returning a
   * populated RequestStatNames if successful. See the implementation
   * (source/common/connect/common.h) for the definition of RequestStatNames. It is
  ...
  /**
   * Parses out request connect service-name and method from the path with dots
   * replaced by underscores in connect service-name, returning a populated
   * RequestStatName if successful. See
   * the implementation (source/common/connect/common.h) for the definition of
   ...
   * @param connect_status supplies the Connect status.
   */
  virtual void chargeStat(const Upstream::ClusterInfo& cluster, Protocol protocol,
                          ...
                          const Http::HeaderEntry* connect_status) PURE;
  ...
  /**
   * @return a struct containing StatNames for Connect stat tokens.
   */

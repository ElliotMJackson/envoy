...
/**
 * An in-flight Connect unary RPC.
 */
class AsyncRequest {
public:
...
public:
  ...
  /**
   * Called when the async Connect request succeeds. No further callbacks will be invoked.
   * @param response the Connect response bytes.
  ...
  /**
   * Called when the async Connect request fails. No further callbacks will be invoked.
   * @param status the Connect status.
   * @param message the Connect status message or empty string if not present.
   * @param span a tracing span to fill with extra tags.
   */
  virtual void onFailure(Status::ConnectStatus status, const std::string& message,
                         Tracing::Span& span) PURE;
};

/**
 * Notifies caller of async Connect stream status.
 * Note the Connect stream is full-duplex, even if the local to remote stream has been ended by
...
public:
  ...
  /**
   * Called when an async Connect message is received.
   * @param response the Connect message.
  ...
  /**
   * Called when trailing metadata is received. This will also be called on non-Ok connect-status
   * stream termination.
  ...
  /**
   * Called when the remote closes or an error occurs on the Connect stream. The stream is
   ...
   * @param message the Connect status message or empty string if not present.
   */
  virtual void onRemoteClose(Status::ConnectStatus status, const std::string& message) PURE;
};

/**
 * Supports sending Connect requests and receiving responses asynchronously. This can be used to
 * implement either plain Connect or streaming Connect calls.
 */
...
public:
  ...
  /**
   * Start a Connect unary RPC asynchronously.
  ...
  /**
   * Start a Connect stream asynchronously.

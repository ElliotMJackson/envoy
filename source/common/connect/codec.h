...
const uint8_t CONNECT_FH_DEFAULT = 0b0u;
// Last bit for a compressed message.
const uint8_t CONNECT_FH_COMPRESSED = 0b1u;

constexpr uint64_t CONNECT_FRAME_HEADER_SIZE = sizeof(uint8_t) + sizeof(uint32_t);
...
public:
  ...
  // Creates a new CONNECT data frame with the given flags and length.
  // @param flags supplies the CONNECT data frame flags.
  // @param length supplies the CONNECT data frame length.
  ...
  // Prepend the Connect frame into the buffer.
  // @param flags supplies the CONNECT data frame flags.
...
// Wire format (http://www.connect.io/docs/guides/wire.html) of CONNECT data frame
// header:
//
// ---------------------------------------------------------------------
...
enum class State {
  // Waiting for decoding the flags (1 byte) of the CONNECT data frame.
  FhFlag,
...
public:
  // Inspects the given buffer with CONNECT data frame and updates the frame count.
...
public:
  // Decodes the given buffer with CONNECT data frame. Drains the input buffer when
  // decoding succeeded (returns true). If the input is not sufficient to make a
  // complete CONNECT data frame, it will be buffered in the decoder. If a decoding
  // error happened, the input buffer remains unchanged.
  // @param input supplies the binary octets wrapped in a CONNECT data frame.

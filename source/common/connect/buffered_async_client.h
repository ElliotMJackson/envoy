...
#include "source/common/connect/buffered_message_ttl_manager.h"
...
// This class wraps bidirectional Connect and provides message arrival guarantee.
...
public:
  BufferedAsyncClient(uint32_t max_buffer_bytes, const Protobuf::MethodDescriptor& service_method,
                      Connect::AsyncStreamCallbacks<ResponseType>& callbacks,
                      const Connect::AsyncClient<RequestType, ResponseType>& client,
  ...
  void onError(uint64_t message_id) {
    ...
    if (message_it == message_buffer_.end() ||
        message_it->second.first != Connect::BufferState::PendingFlush) {
      return;
    }
...
private:
  ...
  Connect::AsyncStreamCallbacks<ResponseType>& callbacks_;
  Connect::AsyncClient<RequestType, ResponseType> client_;
  Connect::AsyncStream<RequestType> active_stream_;

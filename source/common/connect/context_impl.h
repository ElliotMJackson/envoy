...
#include "envoy/connect/context.h"
...
public:
  ...
  void chargeStat(const Upstream::ClusterInfo& cluster, Protocol protocol,
                  ...
                  const Http::HeaderEntry* connect_status) override;
  void chargeStat(const Upstream::ClusterInfo& cluster, Protocol protocol,
  ...
  /**
   * Resolve the Connect service and method from the HTTP2 :path header. Replace dots in the Connect
  ...
  Stats::StatName protocolStatName(Protocol protocol) const {
    return protocol == Context::Protocol::Connect ? connect_ : connect_web_;
  }
...
private:
  ...
  const Stats::StatName connect_;
  const Stats::StatName connect_web_;

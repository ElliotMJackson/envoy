...
public:
  AsyncClientImpl(Upstream::ClusterManager& cm, const envoy::config::core::v3::ConnectService& config,
...
private:
  void streamError(Status::ConnectStatus connect_status, const std::string& message);
  void streamError(Status::ConnectStatus connect_status) { streamError(connect_status, EMPTY_STRING); }

  void cleanup();
  void trailerResponse(absl::optional<Status::ConnectStatus> connect_status,

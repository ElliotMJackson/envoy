...
ContextImpl::ContextImpl(Stats::SymbolTable& symbol_table)
    : stat_name_pool_(symbol_table), connect_(stat_name_pool_.add("connect")),
      connect_web_(stat_name_pool_.add("connect-web")), success_(stat_name_pool_.add("success")),
...
void ContextImpl::chargeStat(const Upstream::ClusterInfo& cluster, Protocol protocol,
                             const absl::optional<RequestStatNames>& request_names,
                             const Http::HeaderEntry* connect_status) {
  if (!connect_status) {
    return;
  }

  absl::string_view status_str = connect_status->value().getStringView();
...
void ContextImpl::chargeStat(const Upstream::ClusterInfo& cluster,
                             const absl::optional<RequestStatNames>& request_names, bool success) {
  chargeStat(cluster, Protocol::Connect, request_names, success);
}

void ContextImpl::chargeRequestMessageStat(const Upstream::ClusterInfo& cluster,
                                           const absl::optional<RequestStatNames>& request_names,
                                           uint64_t amount) {
  Stats::ElementVec elements = statElements(Protocol::Connect, request_names, request_message_count_);
...
void ContextImpl::chargeResponseMessageStat(const Upstream::ClusterInfo& cluster,
                                            const absl::optional<RequestStatNames>& request_names,
                                            uint64_t amount) {
  Stats::ElementVec elements = statElements(Protocol::Connect, request_names, response_message_count_);
  Stats::Utility::counterFromElements(cluster.statsScope(), elements).add(amount);
}
...
void ContextImpl::chargeUpstreamStat(const Upstream::ClusterInfo& cluster,
  ...
  Stats::ElementVec elements = statElements(Protocol::Connect, request_names, upstream_rq_time_);
